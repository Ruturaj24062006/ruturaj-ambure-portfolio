        ...
        <div className="relative border-l border-white/10 ml-4 md:ml-8 space-y-12">
          {experiences.map((exp, idx) => (
            ...
            >
              ...
              <div className="absolute -left-[17px] top-1.5 w-8 h-8 rounded-full border border-white/15 bg-brand-bg-dark flex items-center justify-center group-hover:border-brand-cyan group-hover:shadow-[0_0_15px_rgba(0,229,255,0.4)] transition-all duration-300">
                {exp.icon}
              </div>
              ...
              <div className="glass-card p-6 md:p-8 rounded-2xl relative overflow-hidden">
                {/* Visual Glow */}
                <div className="absolute -top-12 -right-12 w-24 h-24 bg-gradient-to-br from-brand-cyan/5 to-brand-purple/5 blur-xl group-hover:from-brand-cyan/15 group-hover:to-brand-purple/15 transition-all duration-500 rounded-full" />
                ...
                {/* Description Bullets */}
                <ul className="space-y-2.5 text-white/60 text-sm list-none p-0">
                ...
                <div className="mt-6 flex flex-wrap gap-1.5">
                  {exp.tech.map((t) => (
                    <span
                      key={t}
                      className="px-2.5 py-0.5 text-[10px] font-mono rounded bg-white/5 border border-white/5 text-white/50 hover:text-white hover:border-white/15 transition-all"
                    >
                      {t}
                    </span>
                  ))}
